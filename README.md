...
- Muestra notificaciones de éxito/advertencia al ejecutar acciones del menú contextual.
...
Si tu deploy (por ejemplo en Vercel) falla con `No matching version found for @owlbear-rodeo/sdk@...`, cambia la dependencia a `latest` para usar la versión publicada actual del SDK:

```json
"@owlbear-rodeo/sdk": "latest"
```

Luego vuelve a desplegar para que se regenere el lockfile con una versión válida.
